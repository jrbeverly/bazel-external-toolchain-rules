...
def _register_external_toolchain_impl(repository_ctx):
    ...
    repository_ctx.file(
        "BUILD.bazel",
        """load("@bazel-external-toolchain-rules//bazel/macros:toolchains.bzl", "externally_managed_toolchain")
...
# Assume that every toolchain ends with `.toolchain`
def _bazel_toolchain_type_from_file(file):
    filename = file.split(':')[-1]
    name = filename.rstrip('.toolchain')
    return "bazel_toolchain_{}".format(name)

def register_external_toolchains(name, toolchains):
    for toolchain, rule_name in toolchains.items():
        register_external_toolchain(
            # name = _bazel_toolchain_type_from_file(toolchain),
            name = rule_name,
            toolchain = toolchain,
        )
